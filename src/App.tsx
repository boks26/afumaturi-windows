...
import { authApi, boardApi, catalogApi } from './services/desktopApi';
...
export default function App() {
  ...
  const roleFromApi = (roles: string[]) =>
    roles.includes('administrator') || roles.includes('admin') ? 'admin' : 'operator';

  const loadCatalog = async () => {
    setDataLoading(true);
    setApiError(null);
    try {
      const [apiCategories, apiRecipes, apiProducts, apiResources, apiEmployees, apiReports, apiMovements] = await Promise.all([
        catalogApi.categories(), catalogApi.recipes(), catalogApi.products(), boardApi.resources(),
        boardApi.employees(), boardApi.reports(), boardApi.movements(),
      ]);
      ...
      setApiError(error instanceof Error ? error.message : 'Datele nu au putut fi încărcate.');
    } finally {
  ...
  const handleAddResource = async (newRes: Omit<Resource, 'id'>) => {
    try {
      ...
      setApiError(error instanceof Error ? error.message : 'Resursa nu a putut fi salvată.');
    }
  };
  ...
  const handleEditResource = async (id: string, updated: Partial<Resource>) => {
    try {
      ...
      setApiError(error instanceof Error ? error.message : 'Resursa nu a putut fi actualizată.');
    }
  };
  ...
  const handleDeleteResource = async (id: string) => {
    try {
      ...
      setApiError(error instanceof Error ? error.message : 'Resursa nu a putut fi ștearsă.');
    }
  };

  const handleAddStock = async (resourceId: string, qty: number, priceUnit?: number) => {
    try {
      const result = await boardApi.adjustStock(resourceId, {type: 'intrare', quantity: qty, priceUnit, date: new Date().toISOString().split('T')[0], source: 'Aprovizionare manuală stoc'});
      setResources((prev) => prev.map((r) => r.id === resourceId ? result.resource : r));
      setMovements((prev) => [result.movement, ...prev]);
    } catch (error) {
      setApiError(error instanceof Error ? error.message : 'Intrarea în stoc nu a putut fi salvată.');
    }
  };

  const handleRemoveStock = async (resourceId: string, qty: number) => {
    try {
      const result = await boardApi.adjustStock(resourceId, {type: 'iesire', quantity: qty, date: new Date().toISOString().split('T')[0], source: 'Scădere manuală stoc'});
      setResources((prev) => prev.map((r) => r.id === resourceId ? result.resource : r));
      setMovements((prev) => [result.movement, ...prev]);
    } catch (error) {
      setApiError(error instanceof Error ? error.message : 'Ieșirea din stoc nu a putut fi salvată.');
    }
  };

  // 2. Final Products
  const handleAddProduct = async (newProd: Omit<FinalProduct, 'id' | 'stock'>) => {
    try {
      const product = await catalogApi.createProduct(newProd);
      setProducts((prev) => [...prev, product]);
    } catch (error) {
      setApiError(error instanceof Error ? error.message : 'Produsul nu a putut fi salvat.');
    }
  };

  const handleEditProduct = async (id: string, updated: Partial<FinalProduct>) => {
    try {
      await catalogApi.updateProduct(id, updated);
      setProducts((prev) => prev.map((p) => (p.id === id ? { ...p, ...updated } : p)));
    } catch (error) {
      setApiError(error instanceof Error ? error.message : 'Produsul nu a putut fi actualizat.');
    }
  };
  ...
  const handleDeleteProduct = async (id: string) => {
    try {
      ...
      setApiError(error instanceof Error ? error.message : 'Produsul nu a putut fi șters.');
    }
  };
  ...
  // 3. Employees
  const handleAddEmployee = async (newEmp: Omit<Employee, 'id' | 'totalPaid'>) => {
    try {
      const employee = await boardApi.createEmployee(newEmp);
      setEmployees((prev) => [...prev, employee]);
    } catch (error) {
      setApiError(error instanceof Error ? error.message : 'Angajatul nu a putut fi salvat.');
    }
  };
  ...
  const handleEditEmployee = async (id: string, updated: Partial<Employee>) => {
    try {
      ...
      setApiError(error instanceof Error ? error.message : 'Angajatul nu a putut fi actualizat.');
    }
  };
  ...
  const handleDeleteEmployee = async (id: string) => {
    try {
      ...
      setApiError(error instanceof Error ? error.message : 'Angajatul nu a putut fi dezactivat.');
    }
  };
  ...
  const handlePayEmployee = async (id: string, amount: number) => {
    try {
      ...
      setApiError(error instanceof Error ? error.message : 'Plata nu a putut fi salvată.');
    }
  };
  ...
  const handleAddRecipe = async (newRec: Omit<Recipe, 'id'>) => {
    try {
      ...
      setApiError(error instanceof Error ? error.message : 'Rețeta nu a putut fi salvată.');
    }
  };
  ...
  const handleEditRecipe = async (id: string, updated: Partial<Recipe>) => {
    try {
      ...
      setApiError(error instanceof Error ? error.message : 'Rețeta nu a putut fi actualizată.');
    }
  };
  ...
  const handleDeleteRecipe = async (id: string) => {
    try {
      ...
      setApiError(error instanceof Error ? error.message : 'Rețeta nu a putut fi ștearsă.');
    }
  };
  ...
  const handleAddReport = async (newRep: Omit<ProductionReport, 'id'>) => {
    try {
      ...
      setApiError(error instanceof Error ? error.message : 'Darea de seamă nu a putut fi salvată.');
    }
  };

  const handleEditReport = async (id: string, updated: Partial<ProductionReport>) => {
    try {
      const report = await boardApi.updateReport(id, updated);
      setReports((prev) => prev.map((r) => r.id === id ? report : r));
      if (report.status === 'finalizat') await loadCatalog();
    } catch (error) {
      setApiError(error instanceof Error ? error.message : 'Darea de seamă nu a putut fi actualizată.');
    }
  };

  const handleDeleteReport = async (id: string) => {
    if (reports.find((report) => report.id === id)?.status === 'finalizat') {
      setApiError('O dare de seamă finalizată nu poate fi ștearsă, deoarece stocurile au fost deja contabilizate.');
      return;
    }
    try {
      ...
      setApiError(error instanceof Error ? error.message : 'Darea de seamă nu a putut fi ștearsă.');
    }
  };
  ...
  if (!userRole) {
    return (
      <div id="lock-screen-wrapper" className="min-h-screen bg-stone-950 flex items-center justify-center p-4 relative overflow-hidden selection:bg-amber-500 selection:text-stone-950">
        ...
        <div className="bg-stone-900 border border-amber-900/20 max-w-md w-full rounded-2xl p-8 shadow-2xl space-y-8 animate-scaleIn relative z-10">
          <div className="text-center space-y-3">
            ...
            <div>
              <h2 className="text-2xl font-bold font-sans text-stone-100">Afumături de la Nanu</h2>
              <p className="text-xs text-stone-400 mt-1">Sistem securizat pentru planificare rețete și calcul producție</p>
            </div>
          </div>

          <form className="space-y-4" onSubmit={handleLogin}>
            <p className="text-[10px] uppercase tracking-wider text-amber-500 font-mono text-center font-bold">Autentificare Drupal</p>
            ...
            {apiError && <p className="rounded-lg border border-red-900 bg-red-950/40 p-3 text-xs text-red-300">{apiError}</p>}
            <button
              id="btn-login"
              type="submit"
          ...
          <p className="text-[10px] text-center text-stone-600 font-mono">
            v1.2.0 • Proiectat pentru conformitate cu normele de siguranță alimentară
          </p>
        </div>
      </div>
    );
    ...
    switch (activeTab) {
      ...
      case 'materia_prima':
        return (
          <RawMaterialsManager
            resources={resources}
            onAddResource={(res) => handleAddResource({ ...res, bundle: 'materie_prima' })}
      ...
      case 'condimente':
        return (
          <SpicesManager
            resources={resources}
            onAddResource={(res) => handleAddResource({ ...res, bundle: 'condiment' })}
            onEditResource={handleEditResource}
            onDeleteResource={handleDeleteResource}
            onAddStock={handleAddStock}
            onRemoveStock={handleRemoveStock}
      ...
      case 'alte_cheltuieli':
        return (
          <ExpensesManager
            resources={resources}
            onAddResource={(res) => handleAddResource({ ...res, bundle: 'alta_cheltuiala' })}
            onEditResource={handleEditResource}
            onDeleteResource={handleDeleteResource}
          />
        );
  ...
  return (
    <div className="min-h-screen bg-stone-950 text-stone-200 selection:bg-amber-500 selection:text-stone-950 flex flex-col font-sans">
      ...
      <main className="flex-grow max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 w-full">
        {apiError && (
          <div className="mb-6 flex items-center justify-between rounded-xl border border-red-500/20 bg-red-950/40 p-4 text-xs text-red-300">
            <span>{apiError}</span>
            <button onClick={() => void loadCatalog()} className="font-bold text-amber-500">Reîncearcă</button>
          </div>
        )}
        {dataLoading && <div className="mb-6 text-center text-sm text-amber-500">Se încarcă datele din Drupal…</div>}
        {/* Alert badge if user is Operator and tries admin tabs */}
        {userRole === 'operator' && ['retete', 'materia_prima', 'condimente', 'alte_cheltuieli', 'munca_personalului'].includes(activeTab) && (
          <div className="mb-6 p-4 bg-red-950/40 border border-red-500/20 rounded-xl flex items-center space-x-3 text-red-400 text-xs">
            ...
            <div>
              ...
              <p className="text-stone-400">Sunteți conectat ca Operator Secție. Opțiunile de adăugare/editare/ștergere sunt dezactivate pentru rolul dumneavoastră.</p>
            </div>
          </div>
        )}
      ...
      <footer className="bg-stone-900/30 border-t border-stone-900 py-6 text-center text-[11px] font-mono text-stone-600 mt-12">
        <p>© 2026 Afumături de la Nanu. Proiectat pentru producție optimizată.</p>
      </footer>
    </div>
  );
}
